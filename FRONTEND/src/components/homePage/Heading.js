    ...
    return (
        <div className={` ${styles}`}>
            ...
            <div className='tw-mt-4 tw-flex tw-flex-col  tw-bg-red-30 sm:tw-max-w-[913px] sm:tw-mx-auto'>
                <p className={`btnalign:tw-text-[18px]  tw-text-[10px] 3xs:tw-text-[12px] btn:tw-text-[14.5px] sm:tw-text-[17px] sm:tw-text-center tw-text-richblack-300 ${color}`}>
                    {description}
                </p>
                {!nobuttons &&
                    <div className='tw-flex tw-gap-3 3xs:tw-gap-2 2xs:tw-gap-5 tw-mt-5 btnalign:tw-mx-auto'>
                        <Buttons text={btnobj.btn1.text} active extrasmall className={"tw-cursor-pointer "}  />
                        {btnobj.btn2 && <Buttons text={btnobj.btn2.text} className={""} extrasmall />}
                    </div>
                }
            </div>
        </div>
